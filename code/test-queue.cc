...
int main(int argc,char*argv[]){
    ...
    lzyqueue->SetAttribute("useqCnt",StringValue ("2"));

    int a[8] = {0,0,0,0,0,0,0,0};
    lzyqueue->GetWeight(a);

    std::cout<<"  "<<lzyqueue->w0<<" "<<lzyqueue->w1<<"  "<<lzyqueue->w2<<std::endl;
    std::cout<<"  "<<lzyqueue->m_useqCnt<<std::endl;

    std::cout<<"the weight is "<<std::endl;
    for(uint32_t i=0;i<8;i++){
        std::cout<<lzyqueue->m_weight[i]<<" ";
    }
    std::cout<<std::endl;
    ...
    std::cout<<"the enqueue type is: "<<lzyqueue->GetEnqueueType()<<std::endl;
