...
main (int argc, char *argv[])
{
  ...
  pointToPoint.SetQueue("ns3::DropTailQueue","MaxSize",QueueSizeValue (QueueSize (QueueSizeUnit::BYTES,200000)));
  ...
  clientApps.Start (Seconds (2.12));
  clientApps.Stop (Seconds (10.0));

  Ipv4GlobalRoutingHelper::PopulateRoutingTables ();
  std::cout<<"I want to know the number in node 1 is:"<<sw->GetNDevices()<<std::endl;
//  for(uint32_t  i=0;i<sw->GetNDevices();i++){
//     std::cout<<sw->GetDevice(i)->GetTypeId()<<std::endl;
//  }
  Simulator::Run ();
  Simulator::Destroy ();
  return 0;
